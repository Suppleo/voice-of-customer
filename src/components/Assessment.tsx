...
export const Assessment: React.FC = () => {
  ...
  const isCurrentQuestionAnswered = computed.isQuestionAnswered(currentQuestion.id);
  ...
  const handleNext = () => {
    ...
    console.log('Total questions:', computed.totalQuestions);
    console.log('Is last question:', state.currentQuestionIndex === computed.totalQuestions - 1);
  ...
  const isLastQuestion = state.currentQuestionIndex === computed.totalQuestions - 1;
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">
            Trả lời các câu hỏi dưới đây để đánh giá mức độ trưởng thành của doanh nghiệp bạn
          </p>
        </div>
        ...
        <div className="mt-4 p-4 bg-gray-100 rounded text-sm">
          ...
          <p>Question Index: {state.currentQuestionIndex}</p>
          <p>Total Answers: {state.answers.length}</p>
          <p>Is Last Question: {isLastQuestion ? 'Yes' : 'No'}</p>
          <p>Current Question Answered: {isCurrentQuestionAnswered ? 'Yes' : 'No'}</p>
        </div>

        {/* Question Overview */}
        <div className="mt-12 bg-white rounded-xl p-6 shadow-sm border border-gray-100">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Tổng quan câu hỏi</h3>
